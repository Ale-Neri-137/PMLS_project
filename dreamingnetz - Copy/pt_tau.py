...
def tau_batch_means_plateau(
    ...
    *,
    ...
    tau_hats = np.asarray(tau_hats, dtype=np.float64)
    valid_bs = np.asarray(valid_bs, dtype=np.int64)
    rel = np.abs(np.diff(tau_hats)) / np.maximum(tau_hats[:-1], 1e-12)
    for i in range(len(rel) - (consec - 1)):
        if np.all(rel[i : i + consec] <= rel_tol):
            return (float(tau_hats[i + consec]), int(valid_bs[i + consec]))
    return (np.nan, -1)
...
def tau_int_1d(
    ...
    *,
    ...
    tau_b, b_chosen = tau_batch_means_plateau(
        y2, min_batches=bm_min_batches, rel_tol=bm_rel_tol, consec=bm_consec
    )
    ...
    ok_geyer = bool(np.isfinite(tau_g) and n >= min_n and abs(split_z) < 5.0)
    ok_bm = bool(np.isfinite(tau_b) and n >= min_n and b_chosen > 0 and abs(split_z) < 5.0)
...
def plot_tau_overlay(
    ...
    *,
    ...
    q: float = 0.10,
    ylabel: str = r"$\tau_{\mathrm{int}}$ (samples)",
    xlabel: str = "T",
